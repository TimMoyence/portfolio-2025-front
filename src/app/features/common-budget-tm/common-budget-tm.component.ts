...
const CATEGORY_BUDGET_TYPE: Partial<Record<BudgetCategory, "FIXED" | "VARIABLE">> = {
  "Pockets": "VARIABLE",
  "Autres": "VARIABLE",
  "Loyer": "FIXED",
  "Courses": "VARIABLE",
...
export class CommonBudgetTmComponent {
  ...
  private readonly salaryByMonth = signal<Record<BudgetMonth, { tim: string; maria: string }>>(
  ...
  readonly filteredTransactions = computed(() => {
    ...
    return this.transactions().filter((transaction) => {
      const matchesSearch =
        !search ||
        this.normalizeText(transaction.description).includes(search);
      const matchesCategory =
        category === "ALL" || transaction.category === category;
      const matchesState = state === "ALL" || transaction.state === state;
      const matchesBudgetType =
        budgetType === "ALL" ||
        (budgetType === "FIXED" && this.getBudgetType(transaction.category) === "FIXED") ||
        (budgetType === "VARIABLE" &&
  ...
  readonly transactionViews = computed<BudgetTransactionView[]>(() =>
    this.filteredTransactions().map((transaction) => ({
      id: transaction.id,
      dateLabel: this.formatDate(transaction.completedDate || transaction.startedDate),
  ...
  readonly mariaContribution = computed(() =>
    this.formatCurrency(
      this.transactions()
        .filter((transaction) =>
          this.normalizeText(transaction.description).includes("maria naumenko"),
        )
        .reduce((sum, transaction) => sum + Math.max(transaction.amount, 0), 0),
    ),
  );
  ...
  readonly timSalaryShare = computed(() =>
    this.formatPercentage(this.parseAmount(this.timSalary()), this.totalSalary()),
  );

  readonly mariaSalaryShare = computed(() =>
    this.formatPercentage(this.parseAmount(this.mariaSalary()), this.totalSalary()),
  );
  ...
  private inferCategory(
    ...
    if (description.includes("loick babin") || description.includes("les voutes")) {
      return "Loyer";
    }
  ...
  private loadSalaryByMonth(): Record<BudgetMonth, { tim: string; maria: string }> {
  ...
  private loadOverrides(): Record<string, BudgetCategory> {
    ...
    try {
      const raw = JSON.parse(localStorage.getItem(OVERRIDES_KEY) ?? "{}") as Record<
        string,
