  ...
  describe('en contexte navigateur', () => {
    ...
    beforeEach(async () => {
      await TestBed.configureTestingModule({
        ...
        providers: [{ provide: PLATFORM_ID, useValue: 'browser' }, provideRouter([])],
      }).compileComponents();
    ...
    it('devrait composer les deux teasers .lab-big vers les landings', () => {
      ...
      const links = Array.from(compiled.querySelectorAll('.lab-big a.link-arrow')).map((a) =>
        a.getAttribute('href'),
      );
    ...
    it('devrait recalculer la démo météo au changement de ville', () => {
      const compiled = fixture.nativeElement as HTMLElement;
      const tempBefore = compiled.querySelector('.meteo-temp')?.textContent?.trim();

      component['selectCity']('nice');
      fixture.detectChanges();

      const tempAfter = compiled.querySelector('.meteo-temp')?.textContent?.trim();
      expect(tempAfter).not.toBe(tempBefore);
      expect(tempAfter).toContain('24');

      const active = compiled.querySelector('.city-btn.on')?.textContent?.trim();
    ...
    it('devrait rendre la bande CTA vers /offer et /formations', () => {
      ...
      const ctas = Array.from(band?.querySelectorAll('a') ?? []).map((a) => a.getAttribute('href'));
