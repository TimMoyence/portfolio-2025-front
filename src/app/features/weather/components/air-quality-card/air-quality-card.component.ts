...
@Component({
  ...
  template: `
    <app-metric-card
      ...
      i18n-tooltipContent="weather.aqi.tooltip.content|@@weatherAqiTooltipContent"
    ...
    >
      ...
      @if (airQuality(); as aq) {
        <div class="flex items-baseline gap-2">
          ...
          <span class="rounded-full px-2 py-0.5 text-xs font-medium" [ngClass]="badgeClasses()">
            {{ qualityLabel() }}
          </span>
        </div>
...
export class AirQualityCardComponent {
  ...
  readonly qualityLabel = computed(() => {
    ...
    if (aqi <= 60) return $localize`:weather.aqi.moderate|@@weatherAqiModerate:Modéré`;
    if (aqi <= 80) return $localize`:weather.aqi.poor|@@weatherAqiPoor:Mauvais`;
    if (aqi <= 100) return $localize`:weather.aqi.veryPoor|@@weatherAqiVeryPoor:Très mauvais`;
