...
describe('WeatherAlertsCardComponent', () => {
  ...
  it('devrait retourner les classes de severite correctes', () => {
    ...
    expect(minorClasses['bg-yellow-500/20 border-yellow-400/30 text-yellow-100']).toBeTrue();

    const moderateClasses = component.severityClasses('moderate');
    expect(moderateClasses['bg-orange-500/20 border-orange-400/30 text-orange-100']).toBeTrue();

    const severeClasses = component.severityClasses('severe');
    expect(severeClasses['bg-red-500/20 border-red-400/30 text-red-100']).toBeTrue();

    const extremeClasses = component.severityClasses('extreme');
    expect(extremeClasses['bg-red-700/30 border-red-500/40 text-red-50']).toBeTrue();
  });
