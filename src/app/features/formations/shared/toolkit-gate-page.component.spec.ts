...
function buildGateData(
  overrides: Partial<ToolkitGatePageData> = {},
): ToolkitGatePageData {
  return {
    lead: "Un kit de demarrage concret, offert.",
    items: [
      { label: "30 prompts prets a l'emploi" },
      { label: "Une checklist" },
    ],
...
@Component({
  ...
  template: `
    <app-toolkit-gate-page [data]="data" [formationSlug]="slug">
      <ng-container title
        >Le toolkit IA pour <em>solopreneurs</em>.</ng-container
      >
...
describe("ToolkitGatePageComponent", () => {
  ...
  it("devrait projeter le titre (avec accent) dans l'unique h1", () => {
    ...
    expect(headings[0]?.querySelector("em")?.textContent).toContain(
      "solopreneurs",
    );
  });
  ...
  it("devrait rendre le contenu editorial (contents + FAQ)", () => {
    ...
    expect(compiled.querySelectorAll(".tk-list li").length).toBe(
      host.data.contents.length,
    );
    expect(compiled.querySelectorAll(".tk-faq .tk-qa").length).toBe(
      host.data.faq.length,
    );
    // FAQ AEO : une question = un <h3>.
    expect(
      compiled.querySelectorAll(".tk-faq h3").length,
  ...
  it("devrait deriver des id d'en-tete uniques depuis le slug", () => {
    ...
    const compiled = fixture.nativeElement as HTMLElement;
    expect(
      compiled.querySelector("#toolkit-automatiser-avec-ia-contents-heading"),
    ).not.toBeNull();
    expect(
      compiled.querySelector("#toolkit-automatiser-avec-ia-faq-heading"),
    ).not.toBeNull();
  });
});
