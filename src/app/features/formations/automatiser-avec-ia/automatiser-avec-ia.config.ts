...
export const automatiserAvecIaFormation: FormationConfig = {
  ...
  metadata: {
    ...
    description: {
      fr: $localize`:@@formations.automatiser-avec-ia.metadata.description.fr:Formation gratuite de 25 minutes pour entrepreneurs, artisans, commercants et solopreneurs qui veulent gagner deux heures par semaine grace a l'intelligence artificielle sans ecrire une ligne de code. On y decouvre cinq workflows testes en conditions reelles pendant six mois : generation d'un devis en moins de trois minutes, reponses aux emails recurrents, publications sur les reseaux sociaux planifiees a la semaine, extraction automatique de donnees de factures, veille hebdomadaire par secteur. Chaque workflow presente le prompt exact utilise, les pieges a eviter, le temps gagne mesure, et la checklist de relecture obligatoire avant envoi au client. Le format est entierement pense pour les profils non-techniques : aucune API, aucun webhook, aucun abonnement paye requis pour demarrer. Les outils mobilises sont ChatGPT, Perplexity, Buffer et Google Docs dans leur version gratuite, accessibles a tous depuis ordinateur ou smartphone.`,
      en: $localize`:@@formations.automatiser-avec-ia.metadata.description.en:Free 25-minute training for entrepreneurs, craftspeople, shop owners and solopreneurs who want to save two hours per week with artificial intelligence without writing a single line of code. Five real-world workflows tested over six months: quote generation in under three minutes, replies to recurring emails, weekly social media posts scheduled in one go, automatic extraction of invoice data, and automated sector-specific weekly monitoring. Each workflow shows the exact prompt used, pitfalls to avoid, the measured time saved, and the mandatory review checklist before sending anything to a client. The format is designed entirely for non-technical profiles: no API, no webhook, no paid subscription required to get started, and no developer needed. Tools used are ChatGPT, Perplexity, Buffer and Google Docs in their free versions, with step-by-step screenshots for every stage, accessible from any computer or smartphone.`,
    },
    tagline: {
