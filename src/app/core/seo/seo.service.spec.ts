  ...
  function createMockDocument(): Document {
    ...
    const head = {
      appendChild: jasmine
        ...
        .and.callFake((el: Record<string, unknown>) => appendedChildren.push(el)),
      querySelector: jasmine.createSpy('head.querySelector').and.callFake((selector: string) => {
        if (selector === 'script[type="application/ld+json"]') {
          return appendedChildren.find((el) => el['type'] === 'application/ld+json') ?? null;
        }
        return null;
      }),
      querySelectorAll: jasmine
        ...
        .and.callFake((selector: string) => {
          if (selector === 'script[type="application/ld+json"]') {
            return appendedChildren.filter((el) => el['type'] === 'application/ld+json');
          }
          return [];
        }),
  ...
  describe('avec un document valide', () => {
    ...
    describe('updateSeoMetadata - cas nominal', () => {
      ...
      it('devrait creer un lien canonical', () => {
        ...
        expect(
          (mockDocument.head as unknown as { appendChild: jasmine.Spy }).appendChild,
        ).toHaveBeenCalled();
      });
    ...
    describe('updateSeoMetadata - suppression de canonical existant', () => {
      it("devrait supprimer le canonical existant avant d'en creer un nouveau", () => {
        ...
        (mockDocument.querySelector as jasmine.Spy).and.returnValue(existingLink);
    ...
    describe('JSON-LD', () => {
      ...
      it('devrait supprimer le script JSON-LD existant avant d en creer un nouveau', () => {
        ...
        const jsonLdScripts = headChildren.filter((el) => el['type'] === 'application/ld+json');
