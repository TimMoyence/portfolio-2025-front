...
const RULE = "« L'IA genere du contenu.<br/><em>Un expert genere des resultats.</em> »";
...
describe('AsiliAiMethodComponent', () => {
  ...
  it('rend une carte .ai-step par etape, avec numero/titre/description', () => {
    ...
    expect(steps[0].querySelector('.ai-t')?.textContent?.trim()).toBe('Un agent par tache');
  ...
  it("affiche le kicker, le titre <h2> et l'accroche quand fournis en inputs", () => {
    ...
    fixture.detectChanges();
    const host = fixture.nativeElement as HTMLElement;
    expect(host.querySelector('.kicker')?.textContent).toContain('Ma maniere de travailler');
    const h2 = host.querySelector<HTMLElement>('h2.ai-method-title');
    expect(h2).not.toBeNull();
    expect(h2?.textContent?.trim()).toBe(HEADING);
    expect(host.querySelector('.ai-method-lead')?.textContent?.trim()).toBe(LEAD);
  });
  ...
  it("rend la regle d'or avec accent teal <em> via innerHTML, dans une <blockquote>", () => {
    ...
    expect(rule?.querySelector('.ai-rule-who')?.textContent?.trim()).toBe(RULE_WHO);
  });
  ...
  describe('avec projection (page hote)', () => {
    ...
    it("projette le kicker, le titre riche (accent <em>) et l'accroche", () => {
      ...
      expect(host.querySelector('.ai-method-lead')?.textContent).toContain('Accroche projetee.');
    });
  });
});
