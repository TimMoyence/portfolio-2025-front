...
describe("AsiliMethodComponent", () => {
  ...
  it("rend chaque titre d'etape dans un <h3>", () => {
    ...
    const titles = Array.from(
      fixture.nativeElement.querySelectorAll(".step h3"),
  ...
  it("rend le numero et l'intitule (.ix) de chaque etape", () => {
    ...
    const nums = Array.from(
      fixture.nativeElement.querySelectorAll(".step .num"),
    ).map((el) => (el as HTMLElement).textContent?.trim());
    const ixs = Array.from(
      fixture.nativeElement.querySelectorAll(".step .ix"),
    ).map((el) => (el as HTMLElement).textContent?.trim());
  ...
  it("n'affiche pas .ix quand `index` est absent", () => {
    ...
    expect(
      fixture.nativeElement.querySelector(".step h3")?.textContent,
    ).toContain("Comprendre");
  });
  ...
  it("affiche le kicker, le titre <h2> et l'intro quand fournis en inputs", () => {
    ...
    ).toContain("La methode");
    const h2 = fixture.nativeElement.querySelector("h2");
    expect(h2?.textContent).toContain("Un fil conducteur");
    expect(
      fixture.nativeElement.querySelector(".method-head__intro")?.textContent,
    ).toContain("Le digital est un levier.");
  });
  ...
  it("n'affiche ni kicker ni titre ni intro quand non fournis", () => {
    ...
    expect(
      fixture.nativeElement.querySelector(".method-head__intro"),
    ).toBeNull();
  });
  ...
  it("reste rendu cote serveur (SSR fail-open : pas d'anim-ready)", () => {
    setup("server");
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelectorAll(".step").length).toBe(
      STEPS.length,
    );
