...
const PILLARS: readonly AsiliPillar[] = [
  {
    ...
    items: ['Audit & cadrage avant tout chantier', 'Developpement Angular / NestJS sur-mesure'],
...
describe('AsiliPillarsComponent', () => {
  ...
  it('applique la variante en classe (services / formations)', () => {
    setup();
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('.pillar.services')).not.toBeNull();
    expect(fixture.nativeElement.querySelector('.pillar.formations')).not.toBeNull();
  });

  it('rend chaque titre de pilier dans un <h3>', () => {
    setup();
    fixture.detectChanges();
    const titles = Array.from(fixture.nativeElement.querySelectorAll('.pillar h3')).map((el) =>
  ...
  it('rend le tag, la description et les points cles de chaque pilier', () => {
    setup();
    fixture.detectChanges();
    const tags = Array.from(fixture.nativeElement.querySelectorAll('.pillar-tag')).map((el) =>
      (el as HTMLElement).textContent?.trim(),
    );
