...
describe('SlideQuoteComponent', () => {
  it("rend la citation, l'auteur et le rôle", () => {
    ...
    expect(root.querySelector('blockquote').textContent).toContain("L'IA n'efface pas");
    expect(root.querySelector('.slide-quote__author').textContent).toContain('Maxime Rivet');
    expect(root.querySelector('.slide-quote__role').textContent).toContain('Founder');
  });
});
